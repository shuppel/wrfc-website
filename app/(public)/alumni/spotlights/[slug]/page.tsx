...
export default async function AlumniSpotlightPage({ params }: AlumniSpotlightPageProps) {
  ...
  // Handle hometown - it might be a string or a location object
  const hometownDisplay = typeof hometown === 'string' 
    ? hometown 
    : hometown && typeof hometown === 'object' && 'address' in hometown
    ? (hometown as { address?: string }).address
    : hometown && typeof hometown === 'object' && 'name' in hometown
    ? (hometown as { name?: string }).name
